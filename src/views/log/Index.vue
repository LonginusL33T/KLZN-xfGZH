                ...
                <van-list v-model="loading" :finished="finished" offset="10" finished-text="没有更多数据了" @load="getData">
                    <div class="data_wrapper" v-for="(item, index) in dataList" :key="index">
                        <div class="item_wrapper">
                            ...
                            <van-row>
                                <van-col span="6">操作用户</van-col>
                                <van-col span="18">{{ item.userName }}</van-col>
                            </van-row>
                            <van-row>
                                <van-col span="6">操作内容</van-col>
                                <van-col span="18">{{ item.operateContent }}</van-col>
                            </van-row>
                            <van-row>
                                <van-col span="6">操作IP</van-col>
                                <van-col span="18">{{ item.ip }}</van-col>
                            </van-row>
                        </div>
                    </div>
                </van-list>
...
import { operationLog } from '@/axios/request'
...
export default {
    ...
    methods: {
        getData() {
            ...
            Global.publicAxiosRequest(params, operationLog, (data, msg) => {
                this.loading = false;
